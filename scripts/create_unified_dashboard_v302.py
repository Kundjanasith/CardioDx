...
def load_thresholds():
    defaults = {"NORM": 0.50, "MI": 0.30, "STTC": 0.30, "CD": 0.25, "HYP": 0.30}
    raw = read_json(V27_THRESHOLDS)

    if not raw:
        return defaults, "fallback_demo_thresholds"

    # Flexible parsing because threshold file format may vary across releases.
    for key in ["screening", "profiles", "thresholds"]:
        if key in raw:
            obj = raw[key]
            if isinstance(obj, dict):
                if all(k in obj for k in defaults):
                    return {k: float(obj.get(k, defaults[k])) for k in defaults}, f"v2.7_thresholds:{key}"
                if "screening" in obj and isinstance(obj["screening"], dict):
                    s = obj["screening"]
                    return {k: float(s.get(k, defaults[k])) for k in defaults}, "v2.7_thresholds:profiles.screening"
...
Path("apps/streamlit_cardiotwin_unified_v302.py").write_text(app, encoding="utf-8")

Path("artifacts/unified_demo_v302").mkdir(parents=True, exist_ok=True)
Path("artifacts/unified_demo_v302/UNIFIED_DASHBOARD_V302_ADDENDUM.md").write_text(
    """# CardioTwin-AI v3.0.2 Unified Clinical Demo Dashboard

## Purpose

This dashboard integrates the frozen CardioTwin-AI v2.7 core, BeatScope v2.8 evidence, and v3.0 clinical pilot workflow into a single presentation layer.
...
1. Live ECG replay
2. Anatomical-style 3D/4D CardioTwin heart twin
3. v2.7 AI/safety/export artifact status
4. BeatScope v2.8 evidence tab
5. Clinical pilot / doctor-in-the-loop tab
6. Unified JSON/HTML export

## Claim Boundary

This is a research-use unified demo dashboard. It does not replace the frozen v2.7 inference dashboard for strict model evaluation and is not a clinical diagnostic device.

## Positioning

Low-cost, safety-calibrated, multi-scale ECG intelligence platform for preliminary screening, visual explanation, and evidence-based referral support.
""",
    encoding="utf-8",
)

print("DONE: Created apps/streamlit_cardiotwin_unified_v302.py")
print("DONE: Created artifacts/unified_demo_v302/UNIFIED_DASHBOARD_V302_ADDENDUM.md")
